...
export default function Contacts() {
  ...
  async function loadContacts() {
    try {
      ...
      console.error('[contacts] load failed after retries:', e)
    } finally {
      setLoading(false)
    }
  }
  ...
  async function openDirectChat(contactId: string) {
    ...
    // Check if a direct group between these two users already exists
    ...
    if (myGroupIds.length > 0) {
      ...
      // Find existing direct group
      const directGroup = shared?.find(
        (row: any) => row.groups?.type === 'direct'
      )
      if (directGroup) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      {/* Header */}
      <div style={{
        background: '#CC0000', padding: '12px 16px',
      ...
      <div style={{ flex: 1, overflowY: 'auto', background: '#fff' }} className="no-scrollbar">
        ...
        {Object.entries(byRole).map(([label, users]) => (
          <div key={label}>
            <div style={{
              padding: '6px 16px', background: '#F0F2F5',
            ...
            {users.map(u => (
              <div
                ...
                style={{
                  ...
                  cursor: 'pointer', borderBottom: '1px solid #F0F2F5', background: '#fff',
                }}
              >
