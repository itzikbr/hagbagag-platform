...
// גוף הרינדור (SheetSections) + עוזרים מיוצאים לשימוש חוזר במסך הרצף (SheetsFeed).
// ══════════════════════════════════════════════════════════════
export const HEADER_RED = '#c0392b'
const SECTION_BG = '#e8d5d3'
const SECTION_COLOR = '#7b2d26'
export const BG = '#F2EDE9'
export const BORDER = '#E5E0DB'
export const GREY = '#8696A0'
...
export interface DocItem { path?: string; url?: string; name?: string; note?: string }
...
export type Gallery = { title: string; imgs: { url: string; note?: string }[] }
...
// בונה ViewData מרשומת דף + מבנה (building). משותף לצפייה ולרצף.
export function buildViewData(sheet: any, building: any): ViewData {
  const content = (building?.work_content ?? {}) as WorkContent
  const materials = (building?.materials ?? {}) as ViewData['materials']
  return {
...
// חותם מחדש תמונות (bucket פרטי 'sheet-images'), עם נפילה לכתובות השמורות.
export async function signImages(items: DocItem[]): Promise<Gallery['imgs']> {
  ...
  try {
    ...
    return list.map(it => ({ url: (it.path && signed[it.path]) || it.url || '', note: it.note })).filter(x => x.url)
  } catch (e) {
    console.error('[sheet] gallery sign failed:', e)
    return list.map(it => ({ url: it.url || '', note: it.note })).filter(x => x.url)
  }
}
...
// ══════════════════════════════════════════════════════════════
// גוף דף הביצוע — כל הסקשנים. read-only. משותף לצפייה ולרצף.
// onOpenGallery מקבל את שם הגלריה ואת הפריטים; מצב הגלריה מנוהל בהורה.
// ══════════════════════════════════════════════════════════════
export function SheetSections({ data, catSort, onOpenGallery }: {
  data: ViewData
  catSort: Record<string, number>
  onOpenGallery: (title: string, items: DocItem[]) => void
}) {
  ...
  const pick = (value?: string, okey?: string): string => {
  ...
  return (
    <>
      ...
      {/* קטגוריות חומרים (מודל ישן) — רק כאלה עם נתונים */}
      ...
      {(() => {
        ...
        return (
          <Section icon="📦" title="חומרים">
            {groups.map(gr => (
              <div key={gr.code}>
                <div style={{ padding: '6px 12px 3px', fontSize: 12, fontWeight: 800, color: GREY, background: '#faf7f2', direction: 'rtl' }}>{gr.name}</div>
                {gr.rows.map((r, i) => {
      ...
      {(photoCount > 0 || sketchCount > 0 || docCount > 0) && (
        <Section icon="📷" title="תיעוד">
          {photoCount > 0 && (
            <button type="button" onClick={() => onOpenGallery('תמונות שטח', doc.photos ?? [])} style={galRow}>
          ...
          {sketchCount > 0 && (
            <button type="button" onClick={() => onOpenGallery('סקיצות', doc.sketch ?? [])} style={galRow}>
...
// ══════════════════════════════════════════════════════════════
// גלריית תמונות — overlay משותף. מצב מנוהל בהורה.
// ══════════════════════════════════════════════════════════════
export function GalleryOverlay({ gallery, galLoading, onClose }: {
  gallery: Gallery | null; galLoading: boolean; onClose: () => void
}) {
  if (!gallery) return null
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 5000, background: 'rgba(0,0,0,0.92)', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', flexShrink: 0, direction: 'rtl' }} onClick={e => e.stopPropagation()}>
        ...
        <button onClick={onClose} style={{ background: 'rgba(255,255,255,0.15)', color: '#fff', border: 'none', borderRadius: '50%', width: 34, height: 34, fontSize: 18, cursor: 'pointer', lineHeight: 1, fontFamily: 'inherit' }}>✕</button>
      </div>
...
export default function ExecutionSheetView() {
  ...
  async function openGallery(title: string, items: DocItem[]) {
    ...
    const imgs = await signImages(items)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      if (cancelled) return
      setData(buildViewData(sheet, bs?.[0]))
      setLoading(false)
    })()
  ...
  async function share() {
    ...
    try {
      if (navigator.share) { await navigator.share({ title: data.name, text, url }); return }
    } catch { /* בוטל */ }
    try { await navigator.clipboard.writeText(`${text}\n${url}`); alert('הקישור הועתק ✓') } catch { alert(url) }
  }
  ...
  return (
    <div dir="rtl" style={{ display: 'flex', flexDirection: 'column', height: '100%', background: BG, fontFamily: 'Heebo, sans-serif' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 12 }} className="no-scrollbar">
        <SheetSections data={data} catSort={catSort} onOpenGallery={openGallery} />
      </div>
