...
4. Google Drive — סידור עבודה (fileId: 14UteHhz5ofMLqPGNU5i8dR-TSiDt0lEt), לשונית "סידור עבודה". איך למצוא את השבוע הנכון: הקובץ מכיל שבועות מהעבר עד העתיד — קרא 50 שורות מהסוף, מצא את התאריך הכי קרוב להיום, וקרא משם 30 שורות לשני הכיוונים. שורות שהתאריך שלהן בתוך 3 ימים מהיום = השבוע הנכון. מי עובד היום: שכירים (עמאד, סמיר, עלי, אסף) תמיד עובדים — תא ריק = לא שובץ, תא עם טקסט = זו העבודה שלהם; קבלני משנה — רק מי שרשום עובד; מנופים — רק אם רשום.

5. Google Drive — אלכסנדרה (fileId: 1hGPxoQFFt5pO5aUu2Et6PMvEYZqHCzMi), לשונית ראשית "פועלים 2026". מבנה: עמודות תאריך, שם, סכום, הערה. עמודת "סכום" = תנועה עתידית/פעילה שטרם ירדה; עמודת "היסטוריה" = ירד בפועל (מסומן ב-✅). שלוף רק שורות שתאריכן בתוך 30 יום קדימה ועד 7 ימים אחורה = רלוונטי עכשיו (אל תקרא יותר מ-50 שורות). אל תקרא שורות מ-2024 או מחודשים רחוקים.
